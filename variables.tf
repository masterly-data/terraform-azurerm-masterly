...
variable "ca_bundle_pem" {
  type        = string
  default     = null
  description = "One or more CA certificates, PEM-encoded and concatenated, trusted install-wide for outbound TLS — the SMTP relay's STARTTLS handshake, pull-connector DSNs, stream push endpoints and a local AI endpoint alike (the same targets allowed_private_egress_cidrs names; this is the trust half, not the reachability half). Mounted into ca-api and ca-workers as a file and pointed to with SSL_CERT_FILE (OpenSSL's default-verify-paths mechanism, read by every outbound TLS connection those apps make — not only the target that needed it). Public material — not customer credentials — but it REPLACES the process's default trust store rather than adding to it: if any of those targets also present a publicly-trusted certificate, concatenate the image's own CA bundle onto yours first, or every other outbound call starts failing verification the moment this apply lands (see the README, \"Trusting a private CA\"). Null (the default) sets nothing — outbound TLS verifies against the image's own trust store exactly as before this input existed."
